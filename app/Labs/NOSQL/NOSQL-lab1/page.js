...
function page() {
  ...
  const getuser = async (data) => {
    ...
    if (res.status === 200) {
      ...
      if (resdata.message === "Welcome , administrator") {
        setsuccess(true)
      }
    }
    if (res.status === 400) {
  ...
  return (
    <>
      ...
      <div className='bg-white/5 z-10 text-white border border-l-0 border-r-0 border-gray-800 font-semibold h-auto p-5 text-xl'>This lab contains a NoSQL injection vulnerability in the login function.
        To solve the lab, perform a NoSQL injection attack that logs in to the application as the <span className='text-red-500'>administrator</span> user.U can use tools like Burp suite or Caido
