    ...
    <>
      ...
      <div className='p-5 flex flex-col gap-6'>
        ...
        <div className='w-full h-[6vh] border border-gray-800  bg-white/5 rounded-xl flex items-center py-2 px-3 gap-5'>
          ...
          <div className='bg-white/8 p-1 rounded-md border cursor-pointer border-gray-600' id="toys" onClick={(e) => { setcategory(e.target.id) }}>Toys</div>
          <div className='bg-white/8 p-1 rounded-md border cursor-pointer border-gray-600' id="fashion" onClick={(e) => { setcategory(e.target.id) }}>Fashion</div>
          <div className='bg-white/8 p-1 rounded-md border cursor-pointer border-gray-600' id="sports" onClick={(e) => { setcategory(e.target.id) }}>Sports</div>
          <div className='bg-white/8 p-1 rounded-md border cursor-pointer border-gray-600' id="wholesale" onClick={(e) => { setcategory(e.target.id) }}>Wholesale</div>
        </div>
        {category && <><div className='text-center text-2xl font-bold'>Categories of {category.toUpperCase()}</div></>}
