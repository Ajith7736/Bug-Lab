  ...
  return (
    <>
      ...
      <div className='p-5 flex flex-col gap-6'>
        ...
        <div className='w-full h-[6vh] border border-gray-800  bg-white/5 rounded-xl flex items-center py-2 px-3 max-[450px]:gap-2 gap-5'>
          {["Electronics", "Toys", "Fashion", "Sports", "Wholesale"].map((cat) => (
            <div key={cat} className='bg-white/8 p-1 max-[450px]:text-xs rounded-md border cursor-pointer border-gray-600' id={cat} onClick={() => { setcategory(cat.toLowerCase()) }}>{cat}</div>
          ))}
        </div>
