...
function Lab(params) {
    ...
    return (
        <div className=' h-[10vh] md:h-[9vh] mt-8 w-[90vw] xl:w-[40vw] xl:landscape:h-[12vh] landscape:h-[20vh] border border-gray-800 rounded-full flex items-center justify-evenly'>
            <div className='flex gap-4 items-center m-10 w-[19%] max-[450px]:w-[25%]'><div>{Progress?.completed ? <div className='text-orange-400  max-[450px]:text-xs'>Solved</div> : <div className=' max-[450px]:text-xs'>Lab</div>}</div>{Progress?.completed ? <FaCheck className='text-orange-400 size-3' /> : <ImLab className='text-white lg:size-6   max-[450px]:size-3' />}</div>
